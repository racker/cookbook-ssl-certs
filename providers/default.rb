# encoding: UTF-8

# http://www.foodcritic.io/#FC017
use_inline_resources

action :create do
  name = new_resource.name
  domain = name.gsub(/\.+/, '_')

  # TODO: This will need to be moved to support the delete action
  dbag = Chef::EncryptedDataBagItem.load('ssl_certs', domain)

  base_path = node['ssl_certs']['base_path']

  file "#{base_path}/certs/#{name}.crt" do
    owner 'root'
    group 'root'
    mode 0644
    content dbag['certificate']
  end

  file "#{base_path}/certs/#{name}_bundle.crt" do
    owner 'root'
    group 'root'
    mode 0644
    content dbag['gd_bundle']
  end

  file "#{base_path}/private/#{name}.key" do
    owner 'root'
    group 'root'
    mode 0644
    content dbag['private_key']
  end

  file "#{base_path}/certs/#{name}_chain.crt" do
    owner 'root'
    group 'root'
    mode 0644
    content [dbag['certificate'], dbag['gd_bundle']].join("\n")
  end
end

action :delete do
end
